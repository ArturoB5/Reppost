...
// Contenido y recompensas del post
$content = $_POST['content'];
$token_reward = 0.00000020;
...
// Manejar los videos adjuntos
if (isset($_FILES['videos'])) {
    $allowed_video_types = ['video/mp4', 'video/webm', 'video/ogg'];
    $video_upload_dir = 'View/Videos/post_videos/'; // Asegúrate de que esta carpeta exista y sea escribible.
    foreach ($_FILES['videos']['tmp_name'] as $key => $tmp_name) {
        $file_type = $_FILES['videos']['type'][$key];
        if (in_array($file_type, $allowed_video_types)) {
            $file_name = uniqid() . "_" . basename($_FILES['videos']['name'][$key]);
            $file_path = $video_upload_dir . $file_name;
            // Mover el video al directorio de subida
            if (move_uploaded_file($tmp_name, $file_path)) {
                // Guardar la ruta en la base de datos
                $video_stmt = $conn->prepare("INSERT INTO post_videos (post_id, video_path) VALUES (?, ?)");
                $video_stmt->execute([$post_id, $file_path]);
            }
        }
    }
}
