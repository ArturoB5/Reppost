...
- Un carácter especial (@, #, $, etc.).">
                ...
                <input id="new_password" name="new_password" class="form-control" required="required" type="password"
                    placeholder="Nueva contraseña" style="width: 100%; box-sizing: border-box;">
                <i id="toggle-password-new" class="fa fa-eye"
                    style="position: absolute; right: 10px; top: 35px; cursor: pointer; color: rgb(97 160 159);"></i>
                <!-- Indicador de fuerza -->
            ...
            <div style="position: relative; margin-bottom: 10px; width: 450px;">
                <label for="confirm_password" style="position: relative; display: inline-block;">Confirmar contraseña
                    <i class="fa fa-info-circle" style="margin-left: 5px; color: #61A09F; cursor: pointer;"
                        title="La contraseña debe coincidir con la nueva contraseña.">
                    </i>
                </label>
                <input id="confirm_password" name="confirm_password" class="form-control" required="required" type="password"
                    placeholder="Confirmar contraseña" style="width: 100%; box-sizing: border-box;">
                <i id="toggle-password-confirm" class="fa fa-eye"
                    style="position: absolute; right: 10px; top: 35px; cursor: pointer; color: rgb(97 160 159);"></i>
            </div>
        ...
        <div class="report-container">
            ...
            $reported_users_query = $conn->prepare("
                SELECT m.member_id, m.firstname, m.lastname, m.image
                FROM members AS m
            ...
            while ($user = $reported_users_query->fetch()) {
                ...
                echo '<strong>' . htmlspecialchars($user['firstname'] . ' ' . $user['lastname']) . '</strong>';
                echo '</div>';
            }
            ?>
        </div>
        ...
        <div class="report-container">
            ...
            $reported_posts_query = $conn->prepare("
                SELECT p.post_id, p.content, m.firstname, m.lastname, m.image
                FROM post AS p
        ...
        <div class="report-container">
            ...
            $reported_comments_query = $conn->prepare("
                SELECT c.comment_id, c.comment_text, m.firstname, m.lastname, m.image
        ...
        <h3>Mis reportes <i class="fa fa-flag-checkered" style="color: rgb(97 160 159)"></i></h3>
        <div class="report-container">
            <?php
