...
<body>
    ...
    switch ($type) {
        case 'post':
            // ======================= POST REPORT =======================
            $query = $conn->prepare("SELECT pr.report_id,pr.report_type,pr.report_date,pr.status,pr.status_response,p.post_id,p.content,p.date_posted,p.token_reward,m.firstname,m.lastname,m.image AS profile_image,pi.image_path
                FROM post_reports pr JOIN post p ON p.post_id = pr.post_id JOIN members m ON p.member_id = m.member_id LEFT JOIN post_images pi ON p.post_id = pi.post_id WHERE pr.report_id = :report_id LIMIT 1");
    ...
    ?>
        ...
        case 'comment':
            // ======================= COMMENT REPORT =======================
            $query = $conn->prepare("SELECT rc.report_id,rc.report_type,rc.report_date,rc.status,rc.status_response,c.comment_id,c.comment_text,c.comment_date,m.firstname,m.lastname,m.image AS user_image
                FROM reports_comment rcJOIN post_comments c ON c.comment_id = rc.comment_idJOIN members m ON m.member_id = c.user_idWHERE rc.report_id = :report_idLIMIT 1");
            $query->bindParam(':report_id', $report_id, PDO::PARAM_INT);
            $query->execute();
            if ($row = $query->fetch(PDO::FETCH_ASSOC)) {
        ...
        case 'user':
            // ======================= USER REPORT =======================
            $stmt = $conn->prepare("SELECT ru.report_id,ru.report_type,ru.report_date,ru.status,ru.status_response,ru.reporter_id,ru.reported_id,m.firstname,m.lastname,m.city,m.country,
                m.email,m.gender,m.username,m.image AS profile_image,m.birthdate,m.mobile,m.work,m.member_id FROM report_users ruJOIN members m ON m.member_id = ru.reported_idWHERE ru.report_id = :report_idLIMIT 1");
            ...
            if ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                ...
                $token_query = $conn->prepare("SELECT SUM(token_reward) AS total_tokens FROM post WHERE member_id = :reported_id");
