		...
		<div class="row">
					...
					?>
						<li class="list-group-item contact-item" data-name="<?php echo strtolower($friend_name); ?>">
							<form method="post" action="" class="d-flex align-items-center justify-content-between w-100">
								...
								<!-- Botón borrar conversación (para ambos) -->
								<button type="submit" name="delete_conversation" value="<?php echo $friend_id; ?>"
									class="btn btn-danger" title="Borra el chat para ambos">
			...
			if (isset($_POST['delete_conversation'])) {
				...
				try {
					...
					$stmt1->execute();
					// Para mensajes que se recibe
					$stmt2 = $conn->prepare("
						UPDATE messages
						SET deleted_for_receiver = 1
						WHERE sender_id   = :friend_id
						  AND receiver_id = :session_id
				...
				<div class="flex-grow-1">
					...
					<center>
						...
						<div id="message-container" style="overflow-y: auto; max-height: 800px;">
							<!-- Aqui se Muestran los mensajes dinamicamente -->
						</div>
					</center>
				</div>
			</div>
