				...
				<div class="panel" style="margin-top: 10px;">
					<div class="panel-body">
						...
						<div class="row">
							<br><br>
							<?php
							$query = $conn->query("SELECT members.member_id, members.firstname, members.lastname, members.image, friends.my_friend_id
                            FROM members JOIN friends ON (members.member_id = friends.my_friend_id OR members.member_id = friends.my_id)
                            WHERE (friends.my_friend_id = '$session_id' OR friends.my_id = '$session_id')
                            AND members.member_id != '$session_id'
                            AND members.member_id NOT IN (SELECT blocked_id FROM blocked_users WHERE user_id = '$session_id')
                            AND members.member_id NOT IN (SELECT user_id FROM blocked_users WHERE blocked_id = '$session_id')
                        ");
