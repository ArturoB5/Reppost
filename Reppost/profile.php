				...
				<div class="col-md-12">
					<div style="position: relative; text-align: center;">
						...
						if ($row['role'] === 'moderador') {
							$fullName .= ' <span style="color:#11a39c; font-style: italic;">(Moderador) <i class="fa-solid fa-shield"></i></span>';
							// Mostrar imagen de moderador
							echo '<img src="View\\Images\\app_images\\mod.png" 
									   alt="Moderador" 
									   title="MODERADOR"
									   style="position: absolute; top: -10px; right: 10px; width: 40px; cursor: pointer;" />';
						} elseif ($row['role'] === 'admin') {
							$fullName .= ' <span style="color:#11a39c; font-style: italic;">(Administrador) <i class="fa-solid fa-star"></i></span>';
							echo '<img src="View\\Images\\app_images\\mod.png" 
									   alt="Administrador" 
									   title="ADMINISTRADOR"
									   style="position: absolute; top: -10px; right: 10px; width: 40px; cursor: pointer;" />';
						...
						//MYSQL
						$token_query = $conn->prepare("SELECT 
						IFNULL((SELECT SUM(token_reward) FROM post            WHERE member_id  = :session_id), 0)
						+ IFNULL((SELECT SUM(token_reward) FROM post_reactions  WHERE user_id  = :session_id), 0)
						+ IFNULL((SELECT SUM(token_reward) FROM comment_reactions WHERE user_id   = :session_id), 0)
						+ IFNULL((SELECT SUM(token_reward) FROM post_comments   WHERE user_id    = :session_id), 0)
						AS total_tokens");
						...
						// SELECT 
						// COALESCE((SELECT SUM(token_reward) FROM post            WHERE member_id  = :session_id), 0)
						// + COALESCE((SELECT SUM(token_reward) FROM post_reactions  WHERE user_id  = :session_id), 0)
						// + COALESCE((SELECT SUM(token_reward) FROM comment_reactions WHERE user_id   = :session_id), 0)
						// + COALESCE((SELECT SUM(token_reward) FROM post_comments   WHERE user_id    = :session_id), 0)
						// AS total_tokens
						// ");
						// $token_query->bindParam(':session_id', $session_id, PDO::PARAM_INT);
						// $token_query->execute();
				...
				<div class="panel" style="border-radius: 30px;">
					<div class="panel-body" style="margin-top: -40px;">
						<div class="col-md-12 text-center">
							...
							$count_query = $conn->prepare("SELECT COUNT(DISTINCT m.member_id) AS total_friends FROM members AS m JOIN friends AS f
							ON (m.member_id = f.my_friend_id OR m.member_id = f.my_id) WHERE (f.my_friend_id = :session_id OR f.my_id = :session_id)
							...
							$query = $conn->query("SELECT m.member_id,m.firstname,m.lastname,m.image,f.my_friend_id FROM members AS m JOIN friends AS f
							ON (m.member_id = f.my_friend_id OR m.member_id = f.my_id)WHERE (f.my_friend_id = '$session_id' OR f.my_id = '$session_id')
							AND m.member_id != '$session_id'
							-- EXCLUYE los usuarios que el session_id haya bloqueado
							AND m.member_id NOT IN (SELECT blocked_id FROM blocked_users WHERE user_id = '$session_id')
							-- EXCLUYE los usuarios que hayan bloqueado al session_id
							AND m.member_id NOT IN (SELECT user_id FROM blocked_users WHERE blocked_id = '$session_id')
							ORDER BY f.my_friend_id DESC LIMIT 6");
							...
							?>
								<div class="col-6 col-sm-4 col-md-2 text-center">
									...
									<a href="delete_friend.php?id=<?php echo $friend_id; ?>&redirect=profile"
